...
const createProduct = async function (req, res) {
  ...
  if (Object.keys(req.body).length == 0) {
    return res
      .status(400)
      .send({ status: false, msg: "Please enter request data to be created" });
  }
  ...
  if (!description)
    return res
      .status(400)
      .send({ status: false, msg: "Please enter description" });
  ...
  if (!currencyId)
    return res
      .status(400)
      .send({ status: false, msg: "currencyId is missing" });

  //currencyFormat
  if (!currencyFormat) {
    return res
      .status(400)
      .send({ status: false, msg: "currencyFormat is missing" });
  }
  if (!/^₹$/.test(currencyFormat))
    return res
      .status(400)
      .send({ status: false, msg: "The  currencyFormat is not valid" });
  ...
  if (!files) {
    return res
      .status(400)
      .send({ status: false, msg: "Please enter productImage" });
  }
  ...
  if (availableSizes) {
    ...
    for (let i = 0; i < availSizes.length; i++) {
      if (!s.includes(availSizes[i]))
        return res
          .status(400)
          .send({ status: false, msg: "provide only S, XS, M, X, L, XXL, XL" });
    }
...
const getProducts = async function (req, res) {
  try {
    ...
    let { priceGreaterThan, priceLessThan, priceSort, name, availableSizes } =
      data;
      ...
      for (let i = 0; i < availSizes.length; i++) {
        if (!s.includes(availSizes[i]))
          return res
            ...
            .send({
              status: false,
              msg: "provide only S, XS, M, X, L, XXL, XL",
            });
      }
    ...
    if (data) {
      if (priceGreaterThan && priceLessThan) {
        const filter = await productModel
          .find({
        ...
        if (filter.length == 0)
          return res
            .status(404)
            .send({ status: false, messege: "no products found" });
        return res
          .status(200)
          .send({ status: true, messege: "products list", data: filter });
      }
      ...
      if (productDetails.length == 0)
        return res
          .status(404)
          .send({ status: false, messege: "No product found" });

      return res
        .status(200)
        .send({ status: true, messege: "Products list", data: productDetails });
    } else {
      const productDetails = await productModel
...
const getProductbyId = async function (req, res) {
  try {
    ...
    if (!productDetail)
      return res
        .status(404)
        .send({ status: false, messege: "product not found!" });
  ...
  try {
    ...
    if (currencyId) {
      if (currencyId != "INR")
        return res
          .status(400)
          .send({ status: false, msg: "The  currencyId is not valid" });
    ...
    if (availableSizes) {
      ...
      for (let i = 0; i < availSizes.length; i++) {
        ...
        if (findProduct.availableSizes.includes(availSizes[i]))
          return res
            .status(400)
            .send({ status: false, msg: "size already exists" });
...
const deleteProduct = async function (req, res) {
  try {
    ...
    if (!productId)
      return res
        .status(400)
        .send({ status: false, messege: "productId is required" });
    ...
    if (alert.isDeleted)
      return res
        .status(409)
        .send({ status: false, messege: "this product is already deleted" });

    const deleteProducts = await productModel.findByIdAndUpdate(
      productId,
    ...
    res
      .status(200)
      .send({ status: true, messege: "Success", data: deleteProducts });
  } catch (err) {
    res.status(500).send({ status: false, message: err.message });
  }
};
