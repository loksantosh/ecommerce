...
const createUser = async function (req, res) {
  try {
    ...
    if (!/^\w[a-zA-Z.\s_]*$/.test(lname))
      return res.status(400).send({ status: false, msg: "The  lname may contain only letters" });
    ...
    if (typeof shipping.city != "string" || shipping.city.trim().length == 0)
      return res.status(400).send({ status: false, msg: " Please enter city as a String" });

    if (!/^\w[a-zA-Z.\s_]*$/.test(shipping.city))
      return res.status(400).send({ status: false, msg: "The city may contain only letters" });
    ...
    return res.status(201).send({
      status: true, message: "User created successfully", data: saveData,
    });
  } catch (err) {
...
const loginUser = async function (req, res) {
  try {
    ...
    return res.status(200).send({
      status: true, messege: "User login successfull", data: { id, token },
    });
  } catch (err) {
...
const updateUser = async (req, res) => {
  try {
    ...
    if (password) {
      ...
      let validPassword = await bcrypt.compare(password, user.password);
      if (validPassword)
        return res.status(400).send({ status: false, messege: "cannot update old password" });
    ...
    if (address) {
      ...
      if (address.shipping) {
        ...
        if (
          typeof shipping.city != "string" || shipping.city.trim().length == 0)
          return res.status(400).send({ status: false, msg: " Please enter city as a String" });
